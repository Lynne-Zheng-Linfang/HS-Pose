...
def compute_degree_cm_mAP(final_results, synset_names, log_dir, degree_thresholds=[360], shift_thresholds=[100], iou_3d_thresholds=[0.1], iou_pose_thres=0.1, use_matches_for_pose=False, eval_recon=False, plot_figure=True):
    ...
    progress = tqdm(final_results, desc="eval", dynamic_ncols=True)
    ...
    data_path = os.path.join(log_dir, 'mAP_data.npz')
    np.savez(data_path, pose_aps=pose_aps, degree_thres_list=degree_thres_list, shift_thres_list=shift_thres_list,
             iou_thres_list=iou_thres_list, iou_3d_aps=iou_3d_aps)
