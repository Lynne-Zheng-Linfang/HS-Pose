...
class geo_transform_loss(nn.Module):
    ...
    def cal_geo_loss_point_old(self, points, p_rot_g, p_rot_r, p_t, g_R, g_t, sym):
        '''
    ...
    def cal_geo_loss_point(self, points, p_rot_g, p_rot_r, p_t, g_R, g_t, sym):
        ...
        points_re_x = torch.sum((points - p_t.unsqueeze(-2)) * p_rot_r.unsqueeze(-2), dim=-1)

        flag = sym[:, 0] == 0
        points_re_x = torch.where(flag.view(-1, 1), points_re_x, torch.zeros_like(points_re_x))
        new_points_re = torch.where(flag.view(-1, 1, 1), points_re, torch.zeros_like(points_re))
        res = self.loss_func(points_re_x, new_points_re[..., 0])
        valid_num = sum(flag)
        res_geo_x = torch.mean(res)
        if valid_num > 0:
            res_geo_x = res_geo_x * points.size(0) / valid_num
        return res_geo_y + res_geo_x
