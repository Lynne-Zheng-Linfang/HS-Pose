...
from tools.rot_utils import to_R_matrices
...
def generate_RT_old(R, f, T, mode, sym):  # generate sRT mat
    bs = T.shape[0]
...
def generate_RT(R: "tensor (bs,3,3)", f: "tensor (bs)", T: "tensor (bs, 3)",
                mode: "str [vec, gt]", sym: "tensor (bs, 1)"):  # generate sRT mat
    bs = T.shape[0]
    res = torch.eye(4,dtype=T.dtype, device=T.device).unsqueeze(0).repeat(bs, 1, 1)
    if mode == "vec":
        f_green, f_red = f[0].reshape(-1), f[1].reshape(-1)
        f_red = torch.where(sym[:, 0] == 1, torch.zeros_like(f_red, device=f_red.device), f_red)
        Rs = to_R_matrices(f_green, f_red, R[0], R[1])
    else:
        Rs = R
    res[:, :3, :3] = Rs
    res[:, :3, 3] = T
...
def get_size(model):
    s = torch.max(model, dim=1)[0] - torch.min(model, dim=1)[0]
    return s

if __name__ == '__main__':
    from scipy.spatial.transform import Rotation

    R = Rotation.random(2).as_matrix()
    R = torch.FloatTensor(R)
    t = np.array([-0.1, 0.1, 0.9])
    t = torch.FloatTensor(np.expand_dims(t, 0)).repeat(2, 1)
    s = np.array([1])
    s = torch.FloatTensor(s).repeat(2)
    ...
    print(pred_RT, pred_RT_new)
    print(torch.allclose(pred_RT, pred_RT_new))
