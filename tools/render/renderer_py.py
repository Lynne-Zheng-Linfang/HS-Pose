...
from tools.render import inout
from tools.render import misc
...
class RendererPython(renderer.Renderer):
  ...
  def add_object(self, obj_id, model_path_or_pts, scale=1.0, pts_file = False,is_path = True,  **kwargs):
    """See base class."""
    ...
    if is_path:
      model = inout.load_ply(model_path_or_pts)
    else:
      model = {'pts': model_path_or_pts}
    ...
    # Use the model texture.
      ...
      model_texture_path = os.path.join(
        os.path.dirname(model_path_or_pts), self.models[obj_id]['texture_file'])
